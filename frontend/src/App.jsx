...
import { 
  ...
  Info, User, Cpu, Layers, ArrowRight, Rocket
} from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <motion.div variants={item} className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-white mb-2">Project Architecture</h2>
          <p className="text-slate-400">Deep Learning Pipeline & System Design</p>
        </div>
        <button 
          onClick={onClose}
      ...
      <div className="grid md:grid-cols-2 gap-6">
        ...
        {/* 1. The Technology (The Core) */}
        <motion.div variants={item} className="bg-[#0F131C] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden group">
          <div className="absolute top-0 right-0 p-32 bg-indigo-500/5 rounded-full blur-3xl -mr-16 -mt-16" />
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-indigo-500/10 rounded-xl text-indigo-400">
              <Cpu size={24} />
            </div>
            <h3 className="text-xl font-bold text-white">The Core Logic</h3>
          </div>
          
          <div className="space-y-6 relative z-10">
            <div className="flex gap-4">
              <div className="mt-1">
                <div className="w-2 h-full bg-gradient-to-b from-indigo-500 to-emerald-500 rounded-full opacity-30" />
              </div>
              <div className="space-y-6">
                <div>
                  <h4 className="text-indigo-300 font-semibold mb-1">Stage 1: The Finder</h4>
                  <p className="text-sm text-slate-400 leading-relaxed">
                    Utilizes <span className="text-white font-mono">YOLOv8 (COCO)</span> to localize objects in the scene with high-speed bounding box regression. Acts as the visual attention mechanism.
                  </p>
                </div>
                <div>
                  <h4 className="text-emerald-300 font-semibold mb-1">Stage 2: The Thinker</h4>
                  <p className="text-sm text-slate-400 leading-relaxed">
                    A custom-trained <span className="text-white font-mono">MobileNetV2</span> classifier that analyzes cropped regions to predict functional affordances (e.g., "Graspable", "Sittable") based on visual features.
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* 2. The Developer (Personalized) */}
        <motion.div variants={item} className="bg-[#0F131C] border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div className="space-y-4">
            <div className="p-4 bg-white/5 rounded-xl border border-white/5">
              <div className="text-xs text-slate-500 uppercase tracking-wider mb-1">Lead Developer</div>
              <div className="text-lg font-bold text-white">Nithin N</div>
            </div>
            <div className="p-4 bg-white/5 rounded-xl border border-white/5">
                <div className="text-xs text-slate-500 uppercase tracking-wider mb-1">Project Scope</div>
                <div className="text-sm text-slate-300">Personal AI Research Initiative</div>
            </div>
             <div className="p-4 bg-white/5 rounded-xl border border-white/5 flex items-center gap-3">
                <div className="p-2 bg-indigo-500/20 rounded-full text-indigo-400"><Layers size={16}/></div>
                <div className="text-sm text-slate-300">Full-Stack Computer Vision Engineering</div>
            </div>
          </div>
        </motion.div>

        {/* 3. Future Roadmap */}
        <motion.div variants={item} className="md:col-span-2 bg-gradient-to-br from-[#0F131C] to-[#131722] border border-white/10 rounded-2xl p-8 shadow-xl">
          <div className="flex items-center gap-3 mb-6">
            <div className="p-3 bg-amber-500/10 rounded-xl text-amber-400">
              <Rocket size={24} />
            </div>
            <h3 className="text-xl font-bold text-white">Future Developments</h3>
          </div>

          <div className="grid md:grid-cols-3 gap-6">
            {[
              { title: "Mobile Native", desc: "Full PWA deployment with offline inference capabilities." },
              { title: "Real-Time Video", desc: "Switching from static capture to 30FPS continuous video analysis stream." },
              { title: "Context Awareness", desc: "Adding scene graphs to understand object relationships (e.g., 'Cup ON Table')." }
            ].map((feat, i) => (
              <div key={i} className="p-4 rounded-xl border border-white/5 bg-black/20 hover:border-white/10 transition">
                <h4 className="font-bold text-slate-200 mb-2 flex items-center gap-2">
                  <ArrowRight size={14} className="text-amber-500" /> {feat.title}
                </h4>
...
export default function App() {
  ...
  async function startCamera() {
    if (currentView !== 'scanner') setCurrentView('scanner');
    setPreviewSrc(null);
    setDetections([]);
    setActiveIdx(null);
    ...
    <div className="fixed inset-0 bg-[#050911] text-slate-200 font-sans selection:bg-indigo-500/30 overflow-hidden flex flex-col">
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative">
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === 'about' ? (
            <div key="about" className="w-full h-full overflow-y-auto custom-scrollbar pt-8 pb-20">
              <AboutView onClose={() => setCurrentView('scanner')} />
            </div>
          ) : (
